...
# 2. 데이터 연동 로직 (NameError 및 KeyError 해결)
...
def load_and_sync_data():
    ...
    months = [m for m in months if float(m) >= 26.01 or float(m) < 26.00]
    
    # [탭 1] 메인 고정 데이터 (NameError 방지를 위해 d 변수 고정)
    d = {
...
with tab2:
    ...
    # 실제 데이터 매핑 (이미지 ca5d1b, ca64f8, ca689d 기준)
    cur = {
    ...
    st.divider()
    # 파이차트 글자 수평 고정 요청 반영
    st.markdown("<div class='section-title'>📊 월간 자산 배분 비중</div>", unsafe_allow_html=True)
    fig_month = px.pie(df_p, names='항목', values='금액', color='항목', color_discrete_sequence=px.colors.qualitative.Pastel)
    fig_month.update_traces(textinfo="label+percent", textposition="inside", insidetextorientation='horizontal')
    fig_month.update_layout(margin=dict(t=0, l=0, r=0, b=0), paper_bgcolor='rgba(0,0,0,0)', showlegend=False)
...
with tab3:
    ...
    with cw:
        ...
        inc_val = d['net_asset'] - d['base_net_asset'] # NameError: summary -> d 로 수정
