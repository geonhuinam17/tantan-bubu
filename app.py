...
st.markdown("""
    ...
    html, body, [class*="css"] {
        ...
        background-color: #adb5bd !important; /* [수정] 배경을 더 확실한 진회색으로 적용 */
    }
    
    /* 섹션 제목 스타일 */
    .section-title { font-size: 20px; font-weight: 700; color: #333; margin-bottom: 15px; }

    /* [수정] 카드 스타일 복구 (하얀색 배경 + 테두리 네모) */
    ...
    /* [수정] 기간 선택 바 '선'만 진한 회색으로 변경 */
    .stSlider [data-baseweb="slider"] > div {
        background-color: transparent !important;
    }
    .stSlider [data-baseweb="slider"] > div > div {
        background-color: #495057 !important; /* 진한 회색 선 */
    }
    ...
    .blue-pill { background-color: #E1F5FE; color: #0288D1; } /* 하락 시 하늘 */
    
    [data-testid="stMetricLabel"] { font-size: 16px; font-weight: 700; color: #666; }
    </style>
    """, unsafe_allow_html=True)

# 2. 구글 시트 실시간 연동 (CSV 게시 링크 활용)
...
def load_data():
    # 실제 시트 연동: df = pd.read_csv(SHEET_CSV_URL)
    summary = {
    ...
    trend_data = [
        {"날짜": "25.08", "순자산": 75767585}, {"날짜": "25.09", "순자산": 84854400},
        {"날짜": "25.10", "순자산": 91706414}, {"날짜": "25.11", "순자산": 90894166},
        {"날짜": "25.12", "순자산": 96985717}, {"날짜": "26.01", "순자산": 108187566},
...
with tab1:
    ...
    with c3:
        # [수정] 순자산 전월 대비 알약 UI 적용 및 하얀 상자 유지
        st.write("**순자산**")
    ...
    with col_l:
        ...
        fig_line.update_layout(yaxis=dict(range=[7000, f_t['순자산_만원'].max() * 1.15], showgrid=True),
    ...
    with col_r:
        ...
        # [해결] Sunburst 대신 Pie 차트로 변경하여 가운데 구멍(Donut) 완벽 구현
        fig_donut = px.pie(df_p, values='금액', names='항목', hole=0.6,
                           color='항목', color_discrete_map={r['항목']: r['색상'] for _, r in df_p.iterrows()})
        # [수정] 작은 칸(보험, ISA)은 밖으로 빼거나 이름만 표시
        fig_donut.update_traces(textinfo='label', textposition='outside')
        fig_donut.update_layout(margin=dict(t=0, l=0, r=0, b=0), paper_bgcolor='rgba(0,0,0,0)', showlegend=False)
...
with tab2:
    ...
    m3.metric("저축률", f"{(d['savings']/d['income']*100):.1f}%")
    st.bar_chart(pd.DataFrame({"금액": [d['income'], d['expense'], d['savings']]}, index=["수입", "지출", "저축"]))
...
with tab3:
    ...
    with col_h:
        ...
        st.markdown(f"<div class='card-container'><h2 style='color:#8E44AD;'>₩ {liq:,.0f}</h2><p>즉시 현금화 가능한 자산 합계입니다.</p></div>", unsafe_allow_html=True)
    with col_w:
        st.markdown("### 👸 왕비(건희) : 목표 달성률")
        p = (d['net_asset'] - d['base_net_asset']) / 100000000
        st.write(f"**🎯 1차 목표 (+1억) 달성률: {p*100:.1f}%**")
