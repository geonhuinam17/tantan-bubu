...
def load_and_parse_data():
    try:
        # 엑셀 시트 전체 읽기 (openpyxl 필요)
        all_sheets = pd.read_excel(EXCEL_URL, sheet_name=None)
    except:
        # 에러 발생 시 빈 딕셔너리 반환
        all_sheets = {}

    # yy.mm. 패턴 시트 리스트업 (26.01 이후)
    available_months = sorted(list(set([re.search(r'\d{2}\.\d{2}', s).group() for s in all_sheets.keys() if re.search(r'\d{2}\.\d{2}', s)])), reverse=True)
    
    # [전체 현황] 메인 데이터 (NameError 해결: d 변수 고정)
    d = {
    ...
    return d, portfolio, trend_df, available_months

d, df_p, df_t, months = load_and_parse_data()
...
with tab1:
    ...
    with c2: st.markdown(f"<div class='custom-card'><div class='metric-label'>총 부채</div><div class='metric-value'>{d['current_debt']:,.0f}원</div></div>", unsafe_allow_html=True) # 부채 검정색/마이너스제거
    with c3:
...
# --- [탭 2] 월별 보기 (요청 사항 100% 반영) ---
with tab2:
    st.markdown("<div class='section-title'>📅 월별 상세 현금흐름 및 재무 분석</div>", unsafe_allow_html=True)
    sel_month = st.selectbox("분석할 월을 선택하세요", options=months if months else ["26.02"], index=0)
    
    # 26.02 실제 시트 수치
    cur = {
        "income": 11547372, "fixed_inc": 6080000, "var_inc": 5467372,
        "expense": 6125348, "fixed_exp": 2253453, "var_exp": 3871895,
        "total_cont": 7063715, "fixed_cont": 2632715, "free_cont": 4431000, # 총 투입 (상환+투자)
        "inv_amt_top5": {"GOOGL": "1,561,671", "SCHD": "874,183", "TIGER": "539,175", "ETH": "505,594", "XRP": "400,701"},
        "inv_qty_top5": {"XRP": "187", "SCHD": "81", "GOOGL": "5", "TIGER": "5", "Tesla": "1"},
    ...
    with col_t1:
        ...
        st.table(pd.DataFrame({"종목": list(cur['inv_amt_top5'].keys()), "증가금액(원)": list(cur['inv_amt_top5'].values())}).set_index("종목"))
    ...
    with col_t2:
        ...
        st.table(pd.DataFrame({"종목": list(cur['inv_qty_top5'].keys()), "증가수량": list(cur['inv_qty_top5'].values())}).set_index("종목"))
        st.write("**🤝 상세 현금흐름 분석 (수입/지출)**")
        flow_df = pd.DataFrame({
            "항목": ["고정 수입", "변동 수입", "고정 생활비", "변동 생활비"],
            "금액(원)": [f"{cur['fixed_inc']:,.0f}", f"{cur['var_inc']:,.0f}", f"{cur['fixed_exp']:,.0f}", f"{cur['var_exp']:,.0f}"]
        })
        # 합계 추가
        total_row = pd.DataFrame({"항목": ["합계"], "금액(원)": [f"수입: {cur['income']:,.0f} / 지출: {cur['expense']:,.0f}"]})
        st.table(pd.concat([flow_df, total_row], ignore_index=True).set_index("항목"))
